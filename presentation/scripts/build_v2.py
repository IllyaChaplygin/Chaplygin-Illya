"""v2 redesign: full-bleed photography, bold sans type, dark accent panels.

The first pass (out.pptx / final.pptx) kept the original deck's "card on cream
paper with a thin border" language. This pass borrows the grammar of the
Ajinomoto reference deck instead — edge-to-edge photo columns, a dark ink
panel for the company blurb, product photography sitting directly on a solid
brick field, price shown as one big number on a dark tag — while keeping our
own palette and keeping the analytical tone (SKU warnings stay on the slide,
per instruction).
...
SRC, DST = "deck.pptx", "v2.pptx"

INK     = RGBColor(0x23, 0x1C, 0x18)   # near-black warm panel colour
INK_60  = RGBColor(0x5A, 0x50, 0x49)
BRICK   = RGBColor(0xB2, 0x39, 0x24)
BRICK_D = RGBColor(0x8F, 0x2A, 0x19)
PAPER   = RGBColor(0xFA, 0xF7, 0xF2)
WHITE   = RGBColor(0xFF, 0xFF, 0xFF)
CREAM   = RGBColor(0xF3, 0xEA, 0xDF)
PEACH   = RGBColor(0xE8, 0xA9, 0x8D)   # label accent on dark panels
INKTXT  = RGBColor(0x23, 0x1C, 0x18)
...
def placeholder_photo(slide, x, y, w, h, note=None):
    ...
    rect(slide, cx - 0.11, cy - bh / 2 - 0.09, 0.22, 0.09, None, rounded=True, adj=0.3,
         line=CREAM, lw=1.5)
    oval(slide, cx - 0.11, cy - bh / 2 - 0.02 + 0.06, 0.22, line=CREAM, lw=1.5)
    text(slide, x, cy + bh / 2 + 0.10, w, 0.22, "ОЧІКУЄМО ФОТО",
         size=9.5, bold=True, color=CREAM, align=PP_ALIGN.CENTER, spacing=1.0)
    if note:
        text(slide, x + 0.1, cy + bh / 2 + 0.34, w - 0.2, 0.3, note,
             size=7, color=RGBColor(0xC9, 0xBC, 0xB2), align=PP_ALIGN.CENTER, line=1.1)
...
def warning_bar(slide, msg, y, h=0.5, x=MARGIN, w=None):
    ...
    text(slide, x + 0.22, y, w - 0.85, h, msg, size=8.3, color=CREAM, line=1.15,
         anchor=MSO_ANCHOR.MIDDLE)
    return y + h
...
def profile_slide(prs, layout, *, page, kicker_label, name_lines, subtitle, fob,
    ...
    name_h = 0.58 * len(name_lines)
    text(slide, MARGIN, y, col_w, name_h,
         [(ln, {"size": 34, "bold": True, "color": INKTXT, "font": "Calibri", "line": 1.0})
    ...
    text(slide, MARGIN + 0.28, y + 0.18, col_w - 0.56, 0.20, "ПРО КОМПАНІЮ",
         size=9.5, bold=True, color=PEACH, spacing=0.8)
    ...
    for i, (val, cap) in enumerate(stats):
        ...
        text(slide, sx + pad, stat_top, sw - pad - 0.1, 0.40, val, size=27, bold=True,
             color=BRICK, font="Calibri")
...
def draw_card(slide, item, x, w, top):
    ...
    text(slide, inner_x + 0.15, price_y + 0.10, inner_w - 0.30, 0.16,
         "ЗБІРНИЙ ВАНТАЖ 17 М³", size=6.3, bold=True, color=PEACH, spacing=0.6)
...
def card_slide(prs, layout, *, page, kicker_label, title, tag, items, warning):
    ...
    text(slide, MARGIN, 0.60, 7.6, 0.55, title, size=30, bold=True, color=INKTXT, font="Calibri")
